...
_STATES = [
    ...
    ('to_department_manager_approved', 'Assigne Director Manager'),
...
class PurchaseRequest(models.Model):
    ...
    def _compute_is_executive_notes(self):
        for rec in self:
            if rec.requested_by == self.env['res.users'].browse(self.env.uid):
                ...
                user = self.env['res.users'].browse(self.env.uid)
                if user.has_group('__export__.res_groups_64'):
                    rec.is_executive_notes = True
                else:
    ...
    def _check_the_date(self):
        ...
        for rec in self:
            if rec.price_alltotal <= 500 and rec.state == 'to_accountant_manager_approved':
                d1 = datetime.datetime.strptime((rec.date_start),'%Y-%m-%d') 
                new_date = d1 + datetime.timedelta(days=6)
                rec.date_finish = new_date
            elif rec.price_alltotal > 500 and rec.price_alltotal <= 5000 and rec.state == 'to_accountant_manager_approved':
                d1 = datetime.datetime.strptime((rec.date_start),'%Y-%m-%d') 
                new_date = d1 + datetime.timedelta(days=6)
                rec.date_finish = new_date
            elif rec.price_alltotal > 5000 and rec.price_alltotal <= 100000 and rec.state == 'to_accountant_manager_approved':
                d1 = datetime.datetime.strptime((rec.date_start),'%Y-%m-%d') 
                new_date = d1 + datetime.timedelta(days=12)
                rec.date_finish = new_date
            elif rec.price_alltotal > 100000 and rec.state == 'to_accountant_manager_approved':
                d1 = datetime.datetime.strptime((rec.date_start),'%Y-%m-%d') 
        ...
        for rec in self:
            if rec.price_alltotal <= 500:
                ...
                if rec.stage == 1 and rec.ontime_stage > 1:
                    ...
                    dd = "late"
                    rec.done1 = dd
                elif (rec.stage ==7 or rec.stage ==8 or rec.stage ==9 or rec.stage ==10)  and rec.ontime_stage > 15:
                    dd = "late"
                    rec.done1 = dd
                elif (rec.stage ==11 or rec.stage ==12 or rec.stage ==13)  and rec.ontime_stage > 12:
                    dd = "late"
                    rec.done1 = dd

...
_STAGES = [
    ...
    (4, 'موافقة الإدارة المالية'),
    (5, 'تحديد قيمة الطلب'),
    (6, 'تجهيز عروض اﻷسعار'),
    (7, 'المواصفات الفنية'),
    (8, 'موافقة المدير التنفيذي'),
    (9, 'تجهيز المذكرة المالية'),
    (10, 'موافقة أمين الصندوق وإمضاء الشيك'),
    (11, 'موافقة رئيس مجلس اﻷمناء أو نائبه'),
    (12, 'صرف الشيك'),
    (13,'شراء المنتج'),
    (14, 'تسليمه للموظف'),
    (15,'مرفوض')
...
class PurchaseRequest(models.Model):
    ...
    def _compute_is_stage(self):
        for rec in self:
            ...
            if usr.has_group('__export__.res_groups_56'):
                rec.is_stage = True
            elif usr.has_group('__export__.res_groups_76'):
                rec.is_stage = True
            else:
    ...
    def _compute_is_warehouse_notes(self):
        for rec in self:
            ...
            #else:
                user = self.env['res.users'].browse(self.env.uid)
                if user.has_group('__export__.res_groups_76'):
                    rec.is_warehouse_notes = True
                else:

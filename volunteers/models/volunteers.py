...
class volunteers(models.Model):
    ...
    email = fields.Char('الإيميل', required=True)
    ...
    village_id = fields.Many2one('govs.villages.village', 'القرية/المنطقة', reqired=True)
    role = fields.Selection(selection=_ROLES, default=1, string='الدور', reqired=True)
    student = fields.Selection(selection=_STUDENTS, string='طالب/خريج', default=1, reqired=True)
    faculty = fields.Selection(selection=_FACULTY, string ='الكلية')
    qualification = fields.Selection(selection=_QUAL, string ='المؤهل')
    qual_other = fields.Char('مؤهل أخر')
    faculty_other = fields.Char('كلية أخرى')
    job = fields.Char('الوظيفة', reqired=True)

...
#]
   ...
   # @api.one
    ...
    @api.one
    @api.depends('total_expenses', 'total_income','education','project_type','gov_id','job','depends','age2','have_project','is_cancer','is_blind','is_amputation','is_virus_C','is_kidney_fail','is_paralysis','is_disability','is_problem','is_weakness','depts','total_depts','is_earth','total_earth','is_cow','total_cow')
    ...
    def _research_state(self):
        ...
        if self.accepted_type == 1:
            if (self.education == 1 and self.project_type == 1 and self.gov_id.id != 2 ) or (self.job == 4) or (self.depends == 2) or (self.age2 > 55) or (self.age2 < 18 ) or (self.have_project == 2) or (self.is_virus_C == True) or (self.is_kidney_fail == True) or (self.is_cancer == True)  or (self.is_blind == True) or (self.is_amputation == True) or (self.is_paralysis == True) or (self.is_disability == True) or (self.is_problem == True) or (self.is_weakness == True) or (inability >= 1000) or (Excess >= 400 ) or (self.depts == 3 and self.total_depts > 10000) or (self.depts == 2 and self.total_depts > 15000) or (self.is_earth == True and self.total_earth >= 20000) or (self.is_cow == True and self.total_cow >= 20000):

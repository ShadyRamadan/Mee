...
_STATESPAPER = [
    ...
    ('responsible_paper', 'IT اعتماد مسئول'),
...
_HOLDING2 = [
    (1, 'ورقة خطأ'),
    (2, 'غير مطابقة للمعايير'),
    (3,'مستند شخص اخر'),
    (4,'لديه املاك او اراضي زراعية')
]
_EDUCATIONAL2 = [
    (1, 'ورقة خطأ'),
    (2, 'غير مطابقة للمعايير'),
...
_CRIMINALCASE = [
    ...
    (2, 'غير مطابقة للمعايير'),
    (3,'مستند شخص اخر'),
    (4,'مر عليه اكثر من شهرين'),
    (5,'غير موجهة لصناع الحياة')
]
_SOCIALSTATUS = [
    (1, 'ورقة خطأ'),
    (2, 'غير مطابقة للمعايير'),
    ...
    (4,'بحث بتاريخ قديم'),
    (5,'غير موجهة لصناع الحياة'),
    (6,'يفيد تلقي المستفيد لتبرعات او اعانات')
]
...
_REASON = [
    (1, 'أمي والمشروع تروسيكل'),
    (2, 'موظف'),
    (3, 'لا يعول'),
    (4, 'العمر أكبر من 55'),
    (5, 'العمر أقل من 18'),
    (6, 'حاصل على مشروع من صناع الحياة'),
    (7, ' C فيروس'),
    (8, 'فشل كلوي'),
    (9, 'Cancer'),
    (10, 'عمى'),
    ...
    (12, 'شلل'),
    (13, 'إعاقة ذهنية'),
    (14, 'مشكلة في اليد أو القدم'),
    (15, 'ضعف نظر شديد'),
    (16, 'العجز اكثر من 1000'),
    (17, 'الادخار اكتر من 400'),
    (18, 'ديون غير مجدولة اكثر من 10000'),
    (19, 'ديون مجدولة اكثر من 15000'),
    (20, 'يمتلك أراضي قيمتها اكثر من 20000'),
    (21, 'يمتلك مواشي قيمتها اكثر من 20000')
]
#_REPORT_STATE = [
 #   (1, 'المتابعة'),
  #  (2, 'التقرير اﻷول'),
...
class needed_cases(models.Model):
    ...
    #@api.model
    #def company_groups(self, state_economy, domain, **kwargs):
        
     #   companies = self.env['needed.cases'].search([]).name_get()
      #  return companies, None

    #_group_by_full = {
     #   'state_economy': company_groups,
    #}
    #_inherit = ['mail.thread', 'ir.needaction_mixin']
    #_columns = {
    ...
    def onchange_gov(self):
        ...
        if self.gov_id:
            ...
            return res
            self.env.cr.execute("SELECT count(id) FROM needed_cases where gov_id ='%s'"  %(self.gov_id.id,))
            res = self.env.cr.fetchone()[0]
            res2 = str(res+1).zfill(3) 
            self.count_code = 'MF'+res2
...
#]
    ...
    def _number1(self):
        ...
        #self.number1 = my_initals
            if (int(my_initals) % 2) == 0:
                d = 2
                self.gender = 2
                #self.env.cr.execute("UPDATE needed_cases SET gender2 = '%s' where id = '%d'" %(d,self.id))
            else:
                d = 1
                self.gender = 1
                #self.env.cr.execute("UPDATE needed_cases SET gender2 = '%s' where id = '%d'" %(d,self.id)) 
        else:
                self.gender = self.gender2
    ...
    def _number2(self):
        ...
        #self.number3 = my_initals3
            d1 = datetime.today()
            if int(my_initals3[4:6]) == 00 or int(my_initals3[4:6]) == 13 or int(my_initals3[4:6]) == 14 or int(my_initals3[4:6]) == 15 or int(my_initals3[4:6]) == 16 or int(my_initals3[4:6]) ==17 or int(my_initals3[4:6]) == 18 or int(my_initals3[4:6]) == 19:
                d = -1
                self.age = -1
                #self.env.cr.execute("UPDATE needed_cases SET age2 = '%s' where id = '%d'" %(d,self.id))
            elif int(my_initals3[6:8]) == 00 or int(my_initals3[6:8]) == 32 or int(my_initals3[6:8]) == 33 or int(my_initals3[6:8]) == 34 or int(my_initals3[6:8]) == 35 or int(my_initals3[6:8]) == 36 or int(my_initals3[6:8]) == 37 or int(my_initals3[6:8]) == 38 or int(my_initals3[6:8]) ==39:
                d = -2
                self.age = -2
                #self.env.cr.execute("UPDATE needed_cases SET age2 = '%s' where id = '%d'" %(d,self.id))
            else: 
   ...
   # @api.one
    ...
    #def _code(self):
     ...
     #       self.env.cr.execute("SELECT count(id) FROM needed_cases where gov_id ='%s'"  %(self.gov_id.id,))
      #      res = self.env.cr.fetchone()[0]
       #     res2 = str(res+1).zfill(3) 
        #    self.count_code = self.gov_id.code+res2
        #else:
         #   self.count_code = str(0).zfill(3)
    ...
    english_name = fields.Char('Full English Name')
    code = fields.Char('كود المستفيد')
    ...
    national_number = fields.Char('National Number', size=14, on_change='_check_value_national')
    passport_number = fields.Char('Passport Number', size=20)
    uncard_number = fields.Char('UN Card Number', size=20)
    uncard_type = fields.Selection(selection=_UNCARD, string='UN Type', Store=True)
    residency_number = fields.Char('رقم الإقامة', size=25)
    validate_type = fields.Selection(selection=_VALIDATE, string='صلاحية اﻹقامة', Store=True)
    date_expiry = fields.Date('تاريخ انتهاء الاقامة')
    ...
    volunteer_id = fields.Many2one(
        'volunteers', 'اسم المتطوع',
        track_visibility='onchange')
    volunteer_phone = fields.Char(related='volunteer_id.mobile_phone', string='الموبيل')
    volunteer_phone2 = fields.Char(related='volunteer_id.mobile_phone2', string='2 الموبيل')
    #volunteer_gender = fields.Char(related='volunteer_id.gender', string="النوع")
    volunteer_id_auth = fields.Many2one(
        'volunteers', 'اسم المعتمد',
        track_visibility='onchange')
    volunteer_phone_auth = fields.Char(related='volunteer_id_auth.mobile_phone', string='الموبيل')
    volunteer_phone2_auth = fields.Char(related='volunteer_id_auth.mobile_phone2', string='2 الموبيل')
    ...
    gender2 = fields.Selection(selection=_GENDER, string='Gender', Store=True)
    age2 = fields.Integer('Age', Store=True )
    ...
    reason_accepted = fields.Selection(selection=_REASON, string='سبب الرفض', compute='_refus', Store=True)
    ...
    phase_project = fields.Selection(selection=_PHASE, string='Phase', Store=True)
    ...
    is_amputation = fields.Boolean(string="بتر يد")
    is_amputation_leg = fields.Boolean(string="بتر قدم")
    is_paralysis = fields.Boolean(string="شلل يد")
    is_paralysis_leg = fields.Boolean(string="شلل قدم")
    is_disability = fields.Boolean(string="إعاقة ذهنية")
    is_problem = fields.Boolean(string="مشكلة في اليد")
    is_problem_leg = fields.Boolean(string="مشكلة في القدم")
    ...
    proj_income_year = fields.Char('تاريخ فتح المشروع')
    proj_first_money = fields.Integer('راس مال المشروع')
    proj_current_money = fields.Integer('راس المال الحالي')
    proj_income_month = fields.Integer(string='الارباح الشهرية')
    ...
    experiences_project = fields.Selection(selection=_EXP_PROJECT, string='الخبرة في المشروع')
    ...
    familypaper_economy_ids = fields.One2many('needed.cases.familypaper2', 'cases_id',
                               'Family Paper to needed cases',
                               readonly=False,
                               copy=True,
                               track_visibility='onchange')
    ...
    tight_life = fields.Text('المشروع هيفرق معاك في ايه ؟')
    ...
    progress = fields.Integer(string='نسبة إكتمال اﻷوراق',compute='change_progress', store=True)
    ...
    reason_nationalpaper = fields.Selection(selection=_NATIONALPAPER, string='سبب رفض المستند')
    agree_nationalpaper2 = fields.Selection(selection=_AGREEPAPER, string = 'اعتماد ')
    reason_nationalpaper2 = fields.Selection(selection=_NATIONALPAPER, string='سبب رفض المستند')
    ...
    reason_criminalcase = fields.Selection(selection=_CRIMINALCASE, string='سبب رفض المستند')
    ...
    reason_eductional = fields.Selection(selection=_EDUCATIONAL2, string='سبب رفض المستند')
    ...
    reason_socialstatus = fields.Selection(selection=_SOCIALSTATUS, string='سبب رفض المستند')
    agree_socialstatus2 = fields.Selection(selection=_AGREEPAPER, string = 'اعتماد')
    reason_socialstatus2 = fields.Selection(selection=_SOCIALSTATUS, string='سبب رفض المستند')
    ...
    holding_status = fields.Selection(selection=_HOLDING, string='حالة الحيازة')
    agree_holdinghusband = fields.Selection(selection=_AGREEPAPER, string = 'اعتماد')
    reason_holdinghusband = fields.Selection(selection=_HOLDING2, string='سبب رفض المستند')
    ...
    holding_type = fields.Selection(selection=_HOLDING3, string='نوع الملف')
    ...
    reason_holdingfather = fields.Selection(selection=_HOLDING2, string='سبب رفض المستند')
    ...
    holding_type2 = fields.Selection(selection=_HOLDING3, string='نوع الملف')
    ...
    reason_holdingwife = fields.Selection(selection=_HOLDING2, string='سبب رفض المستند')
    ...
    reason_insurancehusband = fields.Selection(selection=_INSURANCE2, string='سبب رفض المستند')
    ...
    agree_drugspaper = fields.Selection(selection=_AGREEPAPER, string = 'اعتماد')
    reason_drugspaper = fields.Selection(selection=_DRUGS3, string='سبب رفض المستند')
    ...
    authentication = fields.Binary('المخالصة', attachment=True)
    ...
    shassih_detection = fields.Binary('شهادة الضمان', attachment=True)
    ...
    is_economy = fields.Boolean(string="Is Economy ",
                                 readonly=True)
    number_economy = fields.Integer(string='Number of Economy Reports')
    ...
    def _compute_is_paper(self):
        for rec in self:
            ...
            if usr.has_group('__export__.res_groups_69'):
                rec.is_paper = True
            elif usr.has_group('__export__.res_groups_75'):
                rec.is_paper = True
            else:
                rec.is_paper = False    
    @api.one
    ...
    def _research_state(self):
        ...
        if self.accepted_type == 1:
            if (self.education == 1 and self.project_type == 1 and self.gov_id.id != 2 ) or (self.job == 4) or (self.depends == 2) or (self.age > 55) or (self.age < 18 ) or (self.have_project == 2) or (self.is_virus_C == True) or (self.is_kidney_fail == True) or (self.is_cancer == True)  or (self.is_blind == True) or (self.is_amputation == True) or (self.is_paralysis == True) or (self.is_disability == True) or (self.is_problem == True) or (self.is_weakness == True) or (inability >= 1000) or (Excess >= 400 ) or (self.depts == 3 and self.total_depts > 10000) or (self.depts == 2 and self.total_depts > 15000) or (self.is_earth == True and self.total_earth >= 20000) or (self.is_cow == True and self.total_cow >= 20000):
    ...
    def _refus(self):
        ...
        Excess = self.total_income - self.total_expenses
        if self.accepted2 == 2:
            if (self.education == 1 and self.project_type == 1 ): 
                self.reason_accepted = 1
            elif (self.job == 4):
                self.reason_accepted = 2
            elif (self.depends == 2):
                self.reason_accepted = 3 
            elif (self.age > 55):
                self.reason_accepted = 4             
            elif (self.age < 18 ):
                self.reason_accepted = 5
            elif (self.have_project == 2):
                self.reason_accepted = 6
            elif (self.is_virus_C == True):
                self.reason_accepted = 7
            elif (self.is_kidney_fail == True):  
                self.reason_accepted = 8
            elif (self.is_cancer == True):   
                self.reason_accepted = 9 
            elif (self.is_blind == True):
                self.reason_accepted = 10
            elif (self.is_amputation == True):   
                self.reason_accepted = 11
            elif (self.is_paralysis == True): 
                self.reason_accepted = 12
            elif (self.is_disability == True): 
                self.reason_accepted = 13
            elif (self.is_problem == True):
                self.reason_accepted = 14
            elif (self.is_weakness == True):
                self.reason_accepted = 15
            elif (inability >= 1000):
                self.reason_accepted = 16
            elif (Excess >= 400 ):
                self.reason_accepted = 17
            elif (self.depts == 3 and self.total_depts > 10000):
                self.reason_accepted = 18
            elif (self.depts == 2 and self.total_depts > 15000):
                self.reason_accepted = 19
            elif (self.is_earth == True and self.total_earth >= 20000):
                self.reason_accepted = 20
            elif (self.is_cow == True and self.total_cow >= 20000):
                self.reason_accepted = 21
        ...
        if self.accepted_type == 1:
            if (self.education == 1 and self.project_type == 1 and self.gov_id.id != 2 ) or (self.job == 4) or (self.depends == 2) or (self.age > 55) or (self.age < 18 ) or (self.have_project == 2) or (self.is_virus_C == True) or (self.is_kidney_fail == True) or (self.is_cancer == True)  or (self.is_blind == True) or (self.is_amputation == True) or (self.is_paralysis == True) or (self.is_disability == True) or (self.is_problem == True) or (self.is_weakness == True) or (inability >= 1000) or (Excess >= 400 ) or (self.depts == 3 and self.total_depts > 10000) or (self.depts == 2 and self.total_depts > 15000) or (self.is_earth == True and self.total_earth >= 20000) or (self.is_cow == True and self.total_cow >= 20000):
                ...
                if isinstance(self.id, models.NewId):
                    ...
                    self.env.cr.execute("UPDATE needed_cases SET accepted4 = '%s' where id = '%d'" %(dd,self.id))
                    if self.accepted2 == 1 and self.state == 'research':
                        sss = 'accreditation'
                        self.env.cr.execute("UPDATE needed_cases SET state = '%s' where id = '%d'" %(sss,self.id))
            else:
                self.accepted2 = 1
                dd = 1
                if isinstance(self.id, models.NewId):
    ...
    def change_colore_on_kanban(self):   
        for record in self:
            color = 0
            if record.state_economy == 1:
                color = 2
            elif record.state_economy == 2:
                color = 7
            elif record.state_economy == 3:
                color = 7
            elif record.state_economy == 4:
                color = 3
            elif record.state_economy == 5:
                color = 5
            record.color = color
    @api.one
    @api.depends('national_paper','national_paper2','social_status','social_status2','criminal_case','insurance_husband','eductional_qualification','holding_husband','holding_father','holding_wife')
    def change_progress(self):
        progress_per = 0
        self.progress =  progress_per
        for rec2 in self:
            if rec2.national_paper != False: 
                progress_per += 10
                rec2.progress =  progress_per
            else:
                progress_per = progress_per
                rec2.progress =  progress_per
                    
            if rec2.national_paper2 != False: 
                progress_per += 10
                rec2.progress =  progress_per
            else:
                progress_per = progress_per
            ...
            if rec2.insurance_husband != False: 
                progress_per += 10
                rec2.progress =  progress_per
            else:
                progress_per = progress_per
            ...
            if rec2.eductional_qualification != False: 
                progress_per += 10
                rec2.progress =  progress_per
            else:
                progress_per = progress_per
    ...
    def button_to_papers(self):
        for rec in self:
            if  rec.agree_drugspaper == 2:
                rec.state = 'drugs'
            elif rec.agree_drugspaper == 1:
                rec.state = 'papers'
            else:
			    rec.state = 'drugs'
        return True
    ...
    def button_to_drugs(self):
        for rec in self:
            if rec.state_accreditation == 'responsible_accreditation' and  rec.authorized_opinion == 1:
                rec.state = 'drugs'
            else:
                rec.state = 'accreditation'
        return True
    ...
    def button_execution(self):
        for rec in self:
            if rec.state_paper == 'project_paper' and rec.agree_nationalpaper == 1 and rec.agree_nationalpaper2 ==1 and rec.agree_socialstatus == 1 and rec.agree_socialstatus2 == 1 and rec.agree_criminalcase == 1 and rec.agree_insuranchusband == 1 and rec.agree_eductional == 1 and rec.agree_holdinghusband == 1 and rec.agree_holdingfather == 1 and rec.agree_holdingwife == 1:
               rec.state = 'execution'
            else:
                rec.state = 'papers'
        return True
    ...
    def button_follow(self):
        for rec in self:
            if rec.execution_done == 1:
                rec.state = 'follow'
                self.env.cr.execute("SELECT count(id) FROM needed_cases_economyreports WHERE cases_id = '%s'" %(self.id))
                res = self.env.cr.fetchone()[0]
                if res == 0:
                    ...
                    for x in range(0,4):
                        if x == 0:
                            ...
                            self.env.cr.execute("INSERT INTO needed_cases_economyreports (report_number,cases_id,date_execution,report_date) VALUES ('%s','%s','%s','%s')" %(x+1,self.id,self.date_execution,new_date))    
                        elif x == 1:
                            d1 = datetime.strptime((self.date_execution),'%Y-%m-%d') 
   ...
   # @api.multi
    ...
    def button_to_responsible_paper(self):
        for rec in self:
            if rec.agree_nationalpaper == 1 and rec.agree_nationalpaper2 ==1 and rec.agree_socialstatus == 1 and rec.agree_socialstatus2 == 1 and rec.agree_criminalcase == 1 and rec.agree_insuranchusband == 1 and rec.agree_eductional == 1 and rec.agree_holdinghusband == 1 and rec.agree_holdingfather == 1 and rec.agree_holdingwife == 1:
...
class neededcasesfamilypaper(models.Model):
    ...
    def legacy_doc1_getFilename(self):
        if (not isinstance(self.national_number, bool)) and (not isinstance(self.national_number2, bool)) :
            #and (len(self.code2) > 0)
...
class neededcaseseconomyreports(models.Model):
    ...
    familypaper_economy_ids = fields.One2many('needed.cases.familypaper2', 'cases_economy_id',
                               'Family Data to needed cases',
                               readonly=False,
                               copy=True,
                               track_visibility='onchange')
    
    volunteer_id_report = fields.Many2one(
        'volunteers', 'اسم المتطوع',
        track_visibility='onchange')
    volunteer_phone_report = fields.Char(related='volunteer_id_report.mobile_phone', string='الموبيل')
    volunteer_phone2_report = fields.Char(related='volunteer_id_report.mobile_phone2', string='2 الموبيل')
   ...
   # state_economy = fields.Selection(selection=_STATESECONOMY,
    ...
    #accepted2 = fields.Char('Accepted', readonly=False, compute='_accepted2')
    #country_id = fields.Many2one('res.country', 'Country', required=True, default=66)
    gov_id = fields.Many2one('govs.villages.gov', string = 'Gov',related='cases_id.gov_id', required=True)
    #city_id = fields.Many2one('govs.villages.city', 'City', required=True, on_change="onchange_city(city_id)")
    #village_id = fields.Many2one('govs.villages.village', 'Village', reqired=False)
    ...
    report_date = fields.Date('تاريخ التقرير المتوقع',store=True,default=False)
    report_type = fields.Selection(string='نوع المتابعة',selection=_REPORT)
    #report_state = fields.Selection(string='مرحلة المتابعة',selection=_REPORT_STATE,default=1)
    visit_date = fields.Date('تاريخ التقرير الفعلي')
    ...
    project_profit2 = fields.Float('ربح المشروع' ,store=True)
    ...
    project_saving2 = fields.Float('الادخار', store=True)
    ...
    pay_done = fields.Float('ما تم سداده')
    rest_dept = fields.Float('المتبقي',compute='_rest_dept')
    reason_dept = fields.Char('سبب الدين')
    color = fields.Integer('Color Index', compute="change_colore_on_kanban")
    economy_edit = fields.Boolean(string="Is editable",
                                 compute="_compute_is_editable",
                                 readonly=True)
   
    need_monthly = fields.Boolean(string="كفالة شهرية")
    need_roofs = fields.Boolean(string="أسقف")
    need_blankets = fields.Boolean(string="بطاطين")
    need_bride = fields.Boolean(string="تجهيز عروسة")
    need_dept = fields.Boolean(string="سداد دين")
    need_project = fields.Boolean(string="مشروع صغير")
    need_medicine = fields.Boolean(string="علاج")
    need_drugs = fields.Boolean(string="علاج إدمان")
    need_jakets = fields.Boolean(string="جواكت")
    need_training = fields.Boolean(string="توعية")
    need_education = fields.Boolean(string="محو أمية")
    need_food = fields.Boolean(string="كرتونة غذائية")
    number_meals_before = fields.Integer(string="عدد الوجبات يوميا")
    general_income_before = fields.Float(string="الدخل العام في اﻷسبوع")
    number_hours_before = fields.Integer(string="عدد ساعات العمل في الأسبوع")
    number_meals_after = fields.Integer(string="عدد الوجبات يوميا")
    general_income_after = fields.Float(string="الدخل العام في اﻷسبوع")
    number_hours_after = fields.Integer(string="عدد ساعات العمل في الأسبوع")
    quantity_beans_before = fields.Float(string="(الحبوب والبقول(أرز - عدس - فول")
    quantity_beans_after = fields.Float(string="(الحبوب والبقول(أرز - عدس - فول")
    cost_beans_before = fields.Float(string="(الحبوب والبقول(أرز - عدس - فول")
    cost_beans_after = fields.Float(string="(الحبوب والبقول(أرز - عدس - فول")
    cost_source_beans = fields.Selection(selection=_COST_SOURCE,string="مصدر التكلفة")
    quantity_vegtables_before = fields.Float(string="الخضروات")
    quantity_vegtables_after = fields.Float(string="الخضروات")
    cost_vegtables_before = fields.Float(string="الخضروات")
    cost_vegtables_after = fields.Float(string="الخضروات")
    cost_source_vegtables = fields.Selection(selection=_COST_SOURCE,string="مصدر التكلفة")
    quantity_meat_before = fields.Float(string="(بروتين(لحوم - فراخ - أسماك")
    quantity_meat_after = fields.Float(string="(بروتين(لحوم - فراخ - أسماك")
    cost_meat_before = fields.Float(string="(بروتين(لحوم - فراخ - أسماك")
    cost_meat_after = fields.Float(string="(بروتين(لحوم - فراخ - أسماك")
    cost_source_meat = fields.Selection(selection=_COST_SOURCE,string="مصدر التكلفة")
    quantity_oil_before = fields.Float(string="الزيوت والسمن")
    quantity_oil_after = fields.Float(string="الزيوت والسمن")
    cost_oil_before = fields.Float(string="الزيوت والسمن")
    cost_oil_after = fields.Float(string="الزيوت والسمن")
    cost_source_oil = fields.Selection(selection=_COST_SOURCE,string="مصدر التكلفة")
    quantity_suger_before = fields.Float(string="الشاي والسكر والتوابل")
    quantity_suger_after = fields.Float(string="الشاي والسكر والتوابل")
    cost_suger_before = fields.Float(string="الشاي والسكر والتوابل")
    cost_suger_after = fields.Float(string="الشاي والسكر والتوابل")
    cost_source_suger = fields.Selection(selection=_COST_SOURCE,string="مصدر التكلفة")
    expense_department_before = fields.Integer('إيجار شقة')
    expense_electricty_before = fields.Integer('فواتير كهرباء')
    expense_water_before = fields.Integer('فواتير مياه')
    expense_communication_before = fields.Integer('فواتير اتصالات')
    expense_dept_before = fields.Integer('قسط دين شهري')
    expense_device_before = fields.Integer('أقساط أجهزة')
    expense_home_before = fields.Integer('مصروف البيت')
    expense_mafroshat_before = fields.Integer('مفروشات')
    expense_education_before = fields.Integer('التعليم')
    expense_studying_before = fields.Integer('دروس أولاد')
    expense_associations_before = fields.Integer('جمعيات')
    expense_food_before = fields.Integer('الغذاء')
    expense_medicine_before = fields.Integer('العلاج')
    expense_smoking_before = fields.Integer('التدخين')
    expense_renew_before = fields.Integer('تجديد المنزل')
    expense_other_before = fields.Integer('أخرى')
    expense_department_after = fields.Integer('إيجار شقة')
    expense_electricty_after = fields.Integer('فواتير كهرباء')
    expense_water_after = fields.Integer('فواتير مياه')
    expense_communication_after = fields.Integer('فواتير اتصالات')
    expense_dept_after = fields.Integer('قسط دين شهري')
    expense_device_after = fields.Integer('أقساط أجهزة')
    expense_home_after = fields.Integer('مصروف البيت')
    expense_mafroshat_after = fields.Integer('مفروشات')
    expense_education_after = fields.Integer('التعليم')
    expense_studying_after = fields.Integer('دروس أولاد')
    expense_associations_after = fields.Integer('جمعيات')
    expense_food_after = fields.Integer('الغذاء')
    expense_medicine_after = fields.Integer('العلاج')
    expense_smoking_after = fields.Integer('التدخين')
    expense_renew_after = fields.Integer('تجديد المنزل')
    expense_other_after = fields.Integer('أخرى')
    date_deliver = fields.Date('تاريخ الاستلام')
    trosikle_type = fields.Char(string="نوع التروسيكل")
    carrige_type = fields.Char(string="نوع الحمولة")
    carrige_kilo = fields.Float(string="الحمولة بالكيلو")
    working_daily = fields.Float(string="معدل العمل باليوم")
    working_hours = fields.Float(string="معدل العمل بالساعة")
    battery_problem = fields.Boolean(string="البطارية")
    carbrateer_problem = fields.Boolean(string="الكاربراتير")
    box_problem = fields.Boolean(string="الصندوق")
    wheel_problem = fields.Boolean(string="العجل")
    motor_problem = fields.Boolean(string="الموتور")
    debrige_problem = fields.Boolean(string="الدبرياج")
    fetees_problem = fields.Boolean(string="الغيارات")
    afsha_problem = fields.Boolean(string="العفشة")
    other_problem = fields.Text(string="أخرى")
    spare_parts_find = fields.Char(string="توافر قطع الغيار")
    spare_parts_not = fields.Char(string="قطع الغيار الغير المتوفرة")
    spare_parts_problem = fields.Char(string="تواجه مشكلة مع قطع الغيار")
    spare_parts_buying = fields.Char(string="بتشتريها منين")
    spare_parts_vendor = fields.Char(string="اسم الوكيل")
    maintance_duration = fields.Char(string="بتعمل صيانة كل اد ايه؟")
    maintance_place = fields.Char(string="بتعمل صيانة فين؟")
    oil_quantity = fields.Char(string="ببتستهلك بنزين اد ايه في اليوم؟")
    oil_duration = fields.Char(string="بتغير الزيت كل اد ايه؟")
    space = fields.Char(' ',readonly=True)
 
    @api.one
    @api.depends('project_income_daily', 'project_income_weekly','project_expenses_daily','project_expenses_weekly')
    #@api.depends('education', 'project_type', 'gov_id')
    def _project_profit(self):
        income_daily = self.project_income_daily * 28
        income_weekly = self.project_income_weekly * 4
        expenses_daily = self.project_expenses_daily * 28
        expenses_weekly = self.project_expenses_weekly * 4
        project_profet_1 = ((income_daily + income_weekly) / 2) - ((expenses_daily+expenses_weekly)/2)
        self.project_profit = project_profet_1
        if isinstance(self.id, models.NewId):
            self.selfid = 0
        else:   
            self.env.cr.execute("UPDATE needed_cases_economyreports SET project_profit2 = '%s' WHERE report_number = '%d' and id = %f" %(project_profet_1,self.report_number,self.id))
                            
    @api.one
    @api.depends('project_income_daily', 'project_income_weekly','project_expenses_daily','project_expenses_weekly','home_expenses_daily','home_expenses_weekly')
    #@api.depends('education', 'project_type', 'gov_id')
    def _project_saving(self):
        income_daily = self.project_income_daily * 28
        income_weekly = self.project_income_weekly * 4
        expenses_daily = self.project_expenses_daily * 28
        expenses_weekly = self.project_expenses_weekly * 4
        home_expenses_daily = self.home_expenses_daily * 28
        home_expenses_weekly = self.home_expenses_weekly * 4
        project_saving_1 = ((income_daily + income_weekly) / 2) - ((expenses_daily+expenses_weekly+home_expenses_daily+home_expenses_weekly)/2)
        self.project_saving = project_saving_1
        
        if isinstance(self.id, models.NewId):
            self.selfid = 0
        else:   
            self.env.cr.execute("UPDATE needed_cases_economyreports SET project_saving2 = '%s' WHERE report_number = '%d' and id = %f" %(project_saving_1,self.report_number,self.id))
    
    @api.multi
    def button_delete(self):
        for rec in self:
            self.env.cr.execute("UPDATE needed_cases_economyreports SET report_type=0,project_income_daily=0,project_income_weekly=0,project_expenses_daily=0, project_expenses_weekly=0, home_expenses_daily=0, home_expenses_weekly=0, project_profit=0,project_profit2=0, project_saving=0,project_saving2=0,project_state=0,safety_factor=0,trosikl_type=0,update_type=0, trosikl_problem=0, activity_type=0,preparing_bride=0,patient_care=0,orphan_care=0,pay_dept=0,original_dept=0,pay_done=0, rest_dept=0,  reason_dept=0 WHERE id=%s and report_number=%s and cases_id=%s" %(rec.id,rec.report_number,rec.cases_id.id))
            if rec.report_number == 1 and rec.cases_id.state_economy == 2:
                rec.env.cr.execute("UPDATE needed_cases SET state_economy = 1 WHERE id = '%s' " %(rec.cases_id.id))
            elif rec.report_number == 2 and rec.cases_id.state_economy == 3:
                rec.env.cr.execute("UPDATE needed_cases SET state_economy = 2 WHERE id = '%s' " %(rec.cases_id.id))
            elif rec.report_number == 3 and rec.cases_id.state_economy == 4:
                rec.env.cr.execute("UPDATE needed_cases SET state_economy = 3 WHERE id = '%s' " %(rec.cases_id.id))
            elif rec.report_number == 4 and rec.cases_id.state_economy == 5:
                rec.env.cr.execute("UPDATE needed_cases SET state_economy = 4 WHERE id = '%s' " %(rec.cases_id.id))

        return True
    
    @api.one
    @api.depends('original_dept','pay_done')
    def _rest_dept(self):
        self.rest_dept = self.original_dept - self.pay_done
        
    @api.multi   
    def create(self,vals):
        #Your logic goes here or call your method
       res_id = super(neededcaseseconomyreports, self).create(vals)
       for ss in self:
            if ss.report_number == 1 and ss.cases_id.state_economy == 1:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 2 WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 2 and ss.cases_id.state_economy == 2:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 3 WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 3 and ss.cases_id.state_economy == 3:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 4 WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 4 and ss.cases_id.state_economy == 4:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 5 WHERE id = '%s' " %(ss.cases_id.id))
            #if ss.report_number == 1:
             #   ss.env.cr.execute("UPDATE needed_cases SET number_economy = 1 WHERE id = '%s' " %(ss.cases_id.id))
            #elif ss.report_number == 2:
             #   ss.env.cr.execute("UPDATE needed_cases SET number_economy = 2 WHERE id = '%s' " %(ss.cases_id.id))
            #elif ss.report_number == 3:
             #   ss.env.cr.execute("UPDATE needed_cases SET number_economy = 3 WHERE id = '%s' " %(ss.cases_id.id))
            #elif ss.report_number == 4:
             #   ss.env.cr.execute("UPDATE needed_cases SET number_economy = 4 WHERE id = '%s' " %(ss.cases_id.id))
    ...
    def write(self,vals):
        # Your logic goes here or call your method
        super(neededcaseseconomyreports, self).write(vals)
        for ss in self:
            if ss.report_number == 1 and ss.cases_id.state_economy == 1:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 2 WHERE id = '%s' " %(ss.cases_id.id))
                ss.env.cr.execute("UPDATE needed_cases SET is_economy = True WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 2 and ss.cases_id.state_economy == 2:
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 3 WHERE id = '%s' " %(ss.cases_id.id))
                ss.env.cr.execute("UPDATE needed_cases SET is_economy = True WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 3 and ss.cases_id.state_economy == 3:
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.report_number == 1 and (rec.cases_id.state_economy == 1 or rec.cases_id.state_economy == 2 or rec.cases_id.state_economy == 3 or rec.cases_id.state_economy == 4 or rec.cases_id.state_economy == 5):
                rec.economy_edit = True    
            elif rec.report_number == 2 and (rec.cases_id.state_economy == 2 or rec.cases_id.state_economy == 3 or rec.cases_id.state_economy == 4 or rec.cases_id.state_economy == 5):
                rec.economy_edit = True
            elif rec.report_number == 3 and (rec.cases_id.state_economy == 3 or rec.cases_id.state_economy == 4 or rec.cases_id.state_economy == 5):
                rec.economy_edit = True
            elif rec.report_number == 4 and (rec.cases_id.state_economy == 4 or rec.cases_id.state_economy == 5):
                rec.economy_edit = True
            else:
                rec.economy_edit = False              
   ...
   # @api.multi
    #def write(self):
    #    if self.report_number == 1 and self.report_state == 1:
     #       self.report_state = 2
      #  elif self.report_number == 2 and self.report_state == 2:
       #     self.report_state = 3
        #elif self.report_number == 3 and self.report_state == 3:
         #   self.report_state = 4
        #elif self.report_number == 4 and self.report_state == 4:
         #   self.report_state = 5
...
class neededcasesfamilypaper2(models.Model):
    
    _name = "needed.cases.familypaper2"
    _description = "Needed Cases Family Paper 2"
    
    cases_id = fields.Many2one('needed.cases',
                                 'Needed Cases',
                                 ondelete='cascade', readonly=True, invisible=True)
    cases_economy_id = fields.Many2one('needed.cases.economyreports',
                                       'Needed Caces Report' ,
                                       ondelete='cascade', readonly=True, invisible=True)
    #code2 = fields.Char(related='cases_id.code',
     #                             string='Code',
    ...
    relation = fields.Selection(selection=_RELATION,string='صلة القرابة')
    education_case = fields.Selection(selection=_FAMILYEDUCTIONAL, string='التعليم')
    gender = fields.Selection(selection=_GENDER, string='Gender', Store=True)
    age = fields.Integer('Age', Store=True )
    size = fields.Char('Size')
    education_cost = fields.Float('Education Cost')

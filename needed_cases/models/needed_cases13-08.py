...
class neededcaseseconomyreports(models.Model):
   ...
   # state_economy = fields.Selection(selection=_STATESECONOMY,
    ...
    report_date = fields.Date('تاريخ التقرير',store=True,default=False)
    ...
    def create(self,vals):
       ...
       for ss in self:
            if ss.report_number == 1 and ss.cases_id.state_economy == 'non_visit':
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 'first_visit' WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 2 and ss.cases_id.state_economy == 'first_visit':
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 'second_visit' WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 3 and ss.cases_id.state_economy == 'second_visit':
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 'third_visit' WHERE id = '%s' " %(ss.cases_id.id))
            elif ss.report_number == 4 and ss.cases_id.state_economy == 'third_visit':
                ss.env.cr.execute("UPDATE needed_cases SET state_economy = 'fourth_visit' WHERE id = '%s' " %(ss.cases_id.id))
        # Your logic goes here or call your method
       return res_id

 ...
 #       default=lambda self: self.env.user.context_donation_journal_id)
    ...
    def transfer(self):
        for rec in self:
            ...
            #self.env.cr.execute("SELECT place.tag_id FROM donation_donation inner join donation_place as place on donation_donation.donation_place = place.id where donation_donation.id= '%s'" %(rec.id))
            ...
            #self.env.cr.execute("SELECT method.tag_id FROM donation_donation inner join donation_instrument as method on donation_donation.donation_method = method.id where donation_donation.id= '%s'" %(rec.id))
...
class DonationLine(models.Model):
    ...
    chech_number = recipt_number = fields.Char(string='Check Number', size=32)
    
    vmachine_id = fields.Many2one(
        'donation.vmachine', string='Visa Machine',
        track_visibility='onchange', ondelete='restrict')

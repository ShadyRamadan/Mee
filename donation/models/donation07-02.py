...
#,states={3: [('readonly', True)]},
 #       default=lambda self: self.env.user.context_donation_journal_id)
    ...
    def _prepare_donation_move(self):
        ...
        for donation_line in self.line_ids:
            if donation_line.in_kind:
                continue
            amount_total_company_cur += donation_line.amount
            ...
            if float_compare(
                ...
                credit_2 = donation_line.amount * -1
                debit_2 = 0
                amount_currency_2 = donation_line.amount
    ...
    def _compute_display_name(self):
        for donation in self:
            if donation.state == 1:
                name = _('Draft Donation of %s') % donation.name
            elif donation.state == 2:
                name = _('Transfer Donation of %s') % donation.name
            elif donation.state == 3:
                name = _('Validate Donation of %s') % donation.name
            elif donation.state == 4:
                name = _('Done Donation of %s') % donation.name
            elif donation.state == 5:
                name = _('Cancelled Donation of %s') % donation.name
            else:
...
class DonationLine(models.Model):
    ...
    def _compute_display_name(self):
        for donation in self:
            if donation.state == 1:
                name = _('Donation Line of %s') % donation.id
            elif donation.state == 2:
                name = _('Donation Line of %s') % donation.id
            elif donation.state == 3:
                name = _('Donation Line of %s') % donation.id
    ...
    @api.model
    def get_account_bank_id(self):
        #self.analytic2 = self.analytic_account_id.id
        return self.account_id_bank.id or False 
    ...
    def _prepare_donation_move(self):
        ...
        for donation_line in self:
            ...
            #account_id = donation_line.product_id.property_account_income_id.id
             ...
             #   account_id = donation_line.product_id.categ_id.\
              #      property_account_income_categ_id.id
            account_id = donation_line.get_account_bank_id()
            account_id_type = donation_line.get_type_account_id()
            if not account_id:
                raise UserError(
                    _("Missing income account on product '%s' or on it's "
    ...
    def to_bank(self):
        ...
        for donation in self:
            ...
            if float_is_zero(
                    donation.it_amount, precision_digits=precision):
                raise UserError(_(
                    "Cannot validate the donation of %s because the "
                    "total amount is 0 !") % donation.donation_by.partner_id.name)
    ...
    def done2cancel(self):
        '''from Done state to Cancel state'''
        for donation in self:
            if donation.move_id_bank:
                donation.move_id_bank.button_cancel()
                donation.move_id_bank.unlink()
            donation.state = 3
        return

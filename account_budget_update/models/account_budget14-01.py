...
class crossovered_budget(models.Model):
    ...
    advance_budget_line = fields.One2many('advance.request', 'budget_id', 'Advances')
    payment_budget_line = fields.One2many('payment.request', 'budget_id', 'Payment Requests')
    ...
    @api.multi
    @api.depends('advance_budget_line.amount_requested')
            ...
            for line in self:
                for advance in line.advance_budget_line:
                    if isinstance(advance.id, models.NewId):
                        ...
                        if advance.state in  (6,8):
                            line.total_advance += advance.amount_requested
                            
    @api.multi
    @api.depends('payment_budget_line.price_alltotal')
                ...
                for payment in line.payment_budget_line:
                    if isinstance(payment.id, models.NewId):
                        ...
                        if payment.state in  (6,8):
                            line.total_payments += payment.price_alltotal
...
class account_budget_post(models.Model):
    ...
    total_variance = fields.Float(string='Total Variance', compute='_compute_variance_all', store=True)
    total_availability = fields.Float(string='Total Availability', compute='_compute_availability_all', store=True)
    advance_budget_line = fields.One2many('advance.request', 'budget_drc', 'Advances')
    payment_budget_line = fields.One2many('payment.request', 'budget_drc', 'Payment Requests')
...
class crossover_budget_lines(models.Model):
    ...
    advance_budget_line = fields.One2many('advance.request', 'budget_dea', 'Advances')
    payment_budget_line = fields.One2many('payment.request', 'budget_dea', 'Payment Requests')
    total_advance = fields.Float(compute='_compute_amount_advance',string="Total Advance")
    total_payments = fields.Float(compute='_compute_amount_payments',string="Total payments")

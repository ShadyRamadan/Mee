...
_STAGES = [
    (1, 'Advance Request'),
    (2, 'Direct and Head of Department Approve'),
    (3, 'Budget Holder Approve'),
    (4, 'Head of Finance Checked'),
    (5, 'CEO Approved'),
    (6, 'Received'),
    (7, 'Remain'),
    (8, 'Reject')
...
class AdvanceRequest(models.Model):
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.state in (2,3,4,5,6,7,8,9,10):
    ...
    def _track_subtype(self, init_values):
        for rec in self:
            if 'state' in init_values and rec.state == 2:
                return 'advance_request.mt_request_to_approve'
            elif 'state' in init_values and rec.state == 3:
                return 'advance_request.mt_request_to_department_manager_approved'
            elif 'state' in init_values and rec.state == 4:
                return 'advance_request.mt_request_to_accountant_manager_approved'
            elif 'state' in init_values and rec.state == 5:
                return 'advance_request.mt_request_approved'
            elif 'state' in init_values and rec.state == 7:
                return 'advance_request.mt_request_rejected'
    ...
    #assigned_to_2 = fields.Many2one('hr.employee', 'Head of Department2',
     #                              store=True, related='department.manager_id',
      #                            track_visibility='onchange')
    assigned_to_2 = fields.Many2one('res.users', 'Head of Department3',
                                   store=True, related='department.manager_user',
                                  track_visibility='onchange')
